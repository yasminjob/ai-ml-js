...
const historico = csv.csvToJSONInputOutput('./historico-petrobras-pn.csv');
...
const fechamento = 'R$ '+parseFloat(resultados[0]).toFixed(2);
const abertura = 'R$ '+parseFloat(resultados[1]).toFixed(2);
const maxima = 'R$ '+parseFloat(resultados[2]).toFixed(2);
const minima = 'R$ '+parseFloat(resultados[3]).toFixed(2);

console.log(`Cotação das Ações PN da Petrobras para o dia ${dia}/${mes}/${ano}`);
