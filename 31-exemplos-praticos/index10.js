...
const historico = csv.csvToJSONInputOutput('./historico-bitcoin.csv');
...
if(isNaN(resultados[2])) resultados[2] = Math.max(resultados[0], resultados[1]);
if(isNaN(resultados[3])) resultados[3] = Math.min(resultados[0], resultados[1]);

const fechamento = 'R$ '+parseFloat(resultados[0]).toFixed(1);
const abertura = 'R$ '+parseFloat(resultados[1]).toFixed(1);
const maxima = 'R$ '+parseFloat(resultados[2]).toFixed(1);
const minima = 'R$ '+parseFloat(resultados[3]).toFixed(1);

console.log(`Cotação do Bitcoin para o dia ${dia}/${mes}/${ano}`);
