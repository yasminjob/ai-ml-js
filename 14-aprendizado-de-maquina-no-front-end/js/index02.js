...
function predict(selEntrada='') {
	...
	if(tipo=='p') return predictProbability(selEntrada);
	else return predictClass(selEntrada);
}

function predictProbability(selEntrada='') {
	const nomeClasses = retornaClasses();
	let probabilidades = [];
...
function predictClass(selEntrada='') {
	const nomeClasses = retornaClasses();
	let nome = '';
	if(selEntrada.toString().trim().length > 0) {
		const Naive = NaiveBayes(selEntrada);
